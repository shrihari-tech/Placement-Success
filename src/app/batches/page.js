...
export default function BatchModel() {
  ...
  const [showNewBatchModeDropdown, setShowNewBatchModeDropdown] =
    useState(false);
    ...
    if (!editBatchData.batchNo.trim()) {
      ...
      batches.some(
        (b) =>
          ...
          b.batchNo.trim().toLowerCase() ===
            editBatchData.batchNo.trim().toLowerCase()
      )
    ) {
  ...
  const handleChange = (path, value) => {
      ...
      if (!newData.batchNo.trim()) {
        ...
        batches.some(
          (b) =>
            b.id !== newData.id &&
            b.batchNo.trim().toLowerCase() ===
              newData.batchNo.trim().toLowerCase()
        )
      ) {
    ...
    if (path.startsWith("sections.")) {
      ...
      if (start && end) {
        if (start === end) {
          newErrors[`sections.${sec}.endDate`] =
  ...
  useEffect(() => {
    ...
    const completed = batchData.filter(
      (b) =>
        b.sections &&
        new Date(b.sections?.Domain?.endDate) < new Date() &&
        new Date(b.sections?.Aptitude?.endDate) < new Date() &&
  ...
  const validateSearchDates = (start, end) => {
    if (start && end) {
      ...
      if (endDateObj < startDateObj) {
        ...
        setSearchDateError(
          "The starting date cannot be earlier than present date"
        );
        return false;
      }
    }
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      ...
      // Add this block for the Batch Name search dropdown
      if (
        batchDropdownRef.current &&
        !batchDropdownRef.current.contains(event.target)
      ) {
        setShowBatchDropdown(false); // Close batch name dropdown
      }
    }
  ...
  const handleEditBatch = (batchId) => {
    ...
    const batchToEdit = [...batches, ...filteredBatches].find(
    ...
    if (batchToEdit) {
      ...
      const initialData = {
        ...
        sections: {
          Domain: {
            startDate: formatDateForEdit(
              batchToEdit.sections?.Domain?.startDate
            ),
            endDate: formatDateForEdit(batchToEdit.sections?.Domain?.endDate),
          },
          Aptitude: {
            startDate: formatDateForEdit(
              batchToEdit.sections?.Aptitude?.startDate
            ),
            endDate: formatDateForEdit(batchToEdit.sections?.Aptitude?.endDate),
          },
          Communication: {
            startDate: formatDateForEdit(
              batchToEdit.sections?.Communication?.startDate
            ),
            endDate: formatDateForEdit(
              batchToEdit.sections?.Communication?.endDate
            ),
          },
        },
      };
  ...
  const hasChanges = () => {
    ...
    for (const section of sections) {
      if (
        editBatchData.sections[section].startDate !==
          initialEditBatchData.sections[section].startDate ||
        editBatchData.sections[section].endDate !==
          initialEditBatchData.sections[section].endDate
      ) {
        return true;
      }
    }
  ...
  const handleDeleteBatch = (batchId) => {
    ...
    if (searchInitiated) {
      const updatedFilteredBatches = filteredBatches.filter(
        (batch) => batch.id !== batchId
      );
    ...
    const completed = updatedBatches.filter(
  ...
  const handleConfirmDelete = () => {
    ...
    if (deleteConfirmationInput !== batchToDelete.batchNo) {
      setDeleteError(
        "Batch name does not match. Please enter the exact batch name."
      );
      return;
    }
  ...
  const validateForm = () => {
    ...
    if (
      batches.some(
        (b) =>
          b.batchNo.trim().toLowerCase() ===
          newBatch.batchNo.trim().toLowerCase()
      )
    ) {
      newErrors.batchNo = "This batch number already exists";
      isValid = false;
  ...
  const handleAddBatch = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5000/batches/addBatch", {
        method: "POST",
      ...
      addBatch(savedBatch); // update context/state with the response from API
      setShowModal(false);
      resetForm();
      toast.success("Batch added successfully");
    } catch (error) {
  ...
  return (
    <div className="flex min-h-screen mt-16 md:mt-1">
      ...
      <div className="fixed top-15 md:top-0 border-b-2 border-gray-300 flex items-center justify-between bg-white w-full py-9 px-4 md:px-8 z-20">
        <h1 className="fixed text-xl text-gray-800 font-semibold">
          {batchHead}
        </h1>
      </div>
        ...
        <div ref={searchContainerRef} className="p-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-7 w-full">
            ...
            <div className="relative flex-1 bg-[#efeeff] h-36 rounded-[10px] shadow-[0px_10.345px_103.45px_0px_rgba(67,67,67,0.10)]">
              <div className="absolute left-6 top-6 text-gray-700 text-4xl font-bold leading-10">
                {ongoingCount}
              </div>
              <div className="absolute left-6 top-[84px] text-xl text-gray-700 font-normal leading-7">
                Ongoing&nbsp;Count
              </div>
          ...
          >
            <div className="flex flex-row flex-wrap justify-center gap-3 px-2 py-3">
              ...
              <div className="relative" ref={batchDropdownRef}>
                ...
                <FiChevronDown
                  className="absolute top-5 right-3 text-gray-500 cursor-pointer" // Changed from 'pointer-events-none' to 'cursor-pointer'
                  size={16}
                  onClick={(e) => {
                    e.stopPropagation(); // Prevent this click from triggering the input's onClick
                    setShowBatchDropdown(!showBatchDropdown); // Toggle the dropdown
                  }}
                />
                {searchTerm && (
                  <button
                ...
                {showBatchDropdown && batches.length > 0 && (
                  <div className="absolute z-10 w-full text-sm bg-[#f3edf7] border border-gray-300 rounded-md shadow-md max-h-60 overflow-y-auto mt-1">
                    {batches
                      .filter((batch) =>
                        batch.batchNo
                          .toLowerCase()
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredBatches.map((batch, index) => (
                      <tr
                        key={batch.id}
                        className="hover:bg-[#f4f3ff] text-gray-500 hover:text-gray-900 "
                      >
                        ...
                        <td className="px-4 py-3 text-center text-sm whitespace-nowrap">
                          {new Date(batch.sections?.Domain?.endDate) <
                            new Date() &&
                          new Date(batch.sections?.Aptitude?.endDate) <
                            new Date() &&
                          new Date(batch.sections?.Communication?.endDate) <
                            new Date() ? (
                            <Image
                              src="/com.svg"
                              alt="Completed"
      ...
      {showModal && (
        ...
        >
          <div
            className="w-[380px] bg-[#F8FAFD] rounded-[10px] px-6 py-4"
            onClick={(e) => {
              e.stopPropagation();
            }}
          >
      ...
      peer-focus:top-3.5 peer-focus:-translate-y-6 peer-focus:scale-75 peer-focus:font-bold peer-focus:text-[#6750A4]`}
            ...
            <div className="relative mb-4" ref={newBatchDropdownRef}>
              ...
              {formErrors.mode && (
                <p className="text-red-600 text-xs mt-1">{formErrors.mode}</p>
              )}
            </div>
              ...
              <div className="mb-4 rounded-xl bg-[#ece6f0] p-4">
                ...
                <div className="flex gap-4">
                  ...
                  <input
                    type="date"
                    min={
                      editBatchData.sections?.Aptitude?.startDate || todayISO
                    }
              ...
              <div className="mb-4 rounded-xl bg-[#ece6f0] p-4">
                ...
                <div className="flex gap-4">
                  <input
                    ...
                    min={todayISO}
                    value={
                      editBatchData.sections?.Communication?.startDate || ""
                    }
                    onChange={(e) =>
                      handleChange(
        ...
        >
          ...
          >
            ...
            <p className="mb-4 text-gray-700 text-sm">
              ...
              <strong className="text-m">
                {initialEditBatchData?.batchNo || originalBatchNo}
              </strong>
              ?
            </p>
