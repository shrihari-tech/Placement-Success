  ...
  const handleSearch = useCallback(() => {
    ...
    if (selectedStatus) {
      results = results.filter(student => {
        const batch = batchData.find(batch => batch.batchNo === student.batch);
    ...
    <div className="flex min-h-screen mt-16 md:mt-1">
      ...
      <div className={`px-3 pt-20 flex-1 bg-[#F8FAFD] mb-12`} ref={searchContainerRef} >
        ...
        <div className="flex bg-[#ECE6F0] rounded-xl p-1 mb-4 relative">
          ...
          />
          {['Student Data', 'Scores', 'Opportunities', 'Batch Change'].map(label => (
            <span
              key={label}
        ...
        <div id="search-container" className="bg-[#F4F3FF] py-3 rounded-xl" tabIndex={0}>
          <div className="flex flex-row justify-center flex-wrap gap-5 py-3">
            ...
            <div className="relative">
              ...
              {showStatusDropdown && (
                <div className="absolute z-10 w-full text-sm bg-[#f3edf7] border border-gray-300 rounded-md shadow-md" ref={statusDropdownRef}>
                  <div
                    key=""
                    tabIndex={0}
                    className="px-4 py-2 cursor-pointer hover:bg-gray-100"
                  ...
                  {['Completed', 'Ongoing'].map(statusOption => (
                    <div
                      ...
                      tabIndex={0}
                      className="px-4 py-2 cursor-pointer hover:bg-gray-100"
                      onClick={() => { setSelectedStatus(statusOption); setShowStatusDropdown(false); }}
                      onKeyDown={(e) => { if (e.key === "Enter") { setSelectedStatus(statusOption); setShowStatusDropdown(false); handleSearch(); } }}
                    >
                      {statusOption}
                    </div>
                  ))}
            ...
            <div className="relative">
              ...
              <FiChevronDown className="absolute top-5 right-3 text-gray-500 pointer-events-none" size={16} />
              {selectedPlacement && (
                <button onClick={(e) => { e.stopPropagation(); setSelectedPlacement(''); setShowPlacementDropdown(false); handleSearch(); }} className="cursor-pointer absolute top-4 right-8 text-gray-500 hover:text-gray-700">
                  <RiCloseCircleLine size={20} />
                </button>
              )}
                ...
                <div className="absolute z-10 w-full text-sm bg-[#f3edf7] border border-gray-300 rounded-md shadow-md" ref={placementDropdownRef}>
                  ...
                  {['Placed', 'Yet to Place', 'Not Placed', 'Not Interested'].map(placementOption => (
                    <div
                      ...
                      tabIndex={0}
                      className="px-4 py-2 cursor-pointer hover:bg-gray-100"
                      onClick={() => { setSelectedPlacement(placementOption); setShowPlacementDropdown(false); }}
                      onKeyDown={(e) => { if (e.key === "Enter") { setSelectedPlacement(placementOption); setShowPlacementDropdown(false); handleSearch(); } }}
                    >
                      {placementOption}
                    </div>
                  ))}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={handleCloseDeleteModal}>
          <div className="w-[500px] bg-[#F8FAFD] rounded-[10px] p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={handleCloseDeleteModal} className="cursor-pointer text-gray-500 hover:text-gray-700">
                <RiCloseCircleLine size={20} />
              </button>
            </div>
            {/* Confirmation Input */}
            <div className="relative mb-6">
              <input
                ...
                required
              />
              <label htmlFor="delete-confirmation" className="absolute px-2 text-sm text-gray-500 duration-300 bg-[#F8FAFD] transform -translate-y-3 scale-75 top-3.5 z-10 origin-[0] left-4 peer-focus:text-xs peer-focus:text-[#6750A4] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-100 peer-focus:-translate-y-6">
                Booking ID
              </label>
            </div>
            {/* Error Message */}
