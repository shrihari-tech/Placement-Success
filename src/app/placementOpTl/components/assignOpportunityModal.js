...
const validateField = (name, value, allData = {}) => {
  ...
  switch (name) {
    case "domain":
      if (!value?.trim()) error = "Domain is required.";
      break;
    case "selectedBatch":
      if (!value?.trim()) error = "Batch is required.";
      break;
    case "companyName":
    ...
    case "driveDate":
      if (!value) {
        ...
        if (isNaN(dateObj.getTime())) {
          ...
          error = "Drive Date cannot be in the past.";
        }
      }
      break;
    case "driveRole":
...
const AssignOpportunityModal = ({
  ...
  const [localApi, contextHolder] = notificationApi
  ...
  const filteredBatches = useMemo(() => {
    ...
    return [];
  }, [localFormData.domain, allStudentData]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { isValid, errors: formErrors } = validateForm(localFormData);
  ...
  return (
    ...
    >
      ...
      <div
        className="w-full max-w-md bg-[#F8FAFD] rounded-[10px] p-6 animate-fade-in-up"
        ...
        <form onSubmit={handleSubmit}>
            ...
            <input
              ...
              value={
                localFormData.domain
                  ? domains.find((d) => d.key === localFormData.domain)
                      ?.label || localFormData.domain
                  : ""
              }
              ...
              } appearance-none focus:outline-none focus:border-[#a17640] peer cursor-pointer`}
            />
            <label
              htmlFor="domain"
              className={`px-2 text-sm ${
                errors.domain ? "text-red-500" : "text-gray-500"
              } duration-300 bg-[#F8FAFD] transform -translate-y-3 scale-75 top-3.5 z-10 origin-[0] left-4 peer-focus:text-xs peer-focus:text-[#a17640] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-100 peer-focus:-translate-y-6`}
            >
              ...
              <div className="absolute z-10 w-full text-sm bg-[#fff8e6] border border-gray-300 rounded-md shadow-md mt-1 max-h-60 overflow-y-auto">
                {domains?.length > 0 ? (
                  domains.map((domainObj) => (
                    <div
                      key={domainObj.key}
                      className={`px-4 py-2 cursor-pointer hover:bg-[#e2d5c5] ${
                        localFormData.domain === domainObj.key
                          ? "bg-[#e8def8]"
                          : ""
                      }`}
                      style={{
            ...
            <input
              ...
              onClick={(e) => {
                if (!localFormData.domain) {
                  setErrors((prev) => ({
                    ...prev,
                    domain: "Please select a domain first.",
                  }));
                  return;
                }
              ...
              className={`block px-4 pb-2 pt-5 w-full text-sm text-gray-900 bg-[#F8FAFD] rounded-sm border-2 ${
                errors.selectedBatch ? "border-red-500" : "border-gray-400"
              } appearance-none focus:outline-none focus:border-[#a17640] peer cursor-pointer ${
                !localFormData.domain ? "opacity-50 cursor-not-allowed" : ""
              }`}
              disabled={!localFormData.domain}
            />
            <label
              htmlFor="selectedBatch"
              className={`px-2 text-sm ${
                errors.selectedBatch ? "text-red-500" : "text-gray-500"
              } duration-300 bg-[#F8FAFD] transform -translate-y-3 scale-75 top-3.5 z-10 origin-[0] left-4 peer-focus:text-xs peer-focus:text-[#a17640] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-100 peer-focus:-translate-y-6`}
            >
                ...
                {filteredBatches.length > 0 ? (
                  filteredBatches.map((batchNo) => (
                    <div
                      key={batchNo}
                      className={`px-4 py-2 cursor-pointer hover:bg-[#e2d5c5] ${
                        localFormData.selectedBatch === batchNo
                          ? "bg-[#e8def8]"
                          : ""
                      }`}
                      style={{
          ...
          <div className="relative mb-2">
            <input
              ...
              id="driveDate"
              className={`block px-4 pb-2 pt-5 w-full text-sm text-gray-900 bg-[#F8FAFD] rounded-sm border-2 ${
                errors.driveDate ? "border-red-500" : "border-gray-400"
              } appearance-none focus:outline-none focus:border-[#a17640] peer`}
            ...
            <label
              htmlFor="driveDate"
              className={`px-2 text-sm ${
                errors.driveDate ? "text-red-500" : "text-gray-500"
              } duration-300 bg-[#F8FAFD] transform -translate-y-3 scale-75 top-3.5 z-10 origin-[0] left-4 peer-focus:text-xs peer-focus:text-[#a17640] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-100 peer-focus:-translate-y-6`}
            >
              Drive Date <span className="text-red-500">*</span>
            </label>
            {errors.driveDate && (
              <p className="text-red-500 text-xs mt-1">{errors.driveDate}</p>
            )}
          </div>
          ...
          <div className="relative mb-2">
            <input
              ...
              id="driveRole"
              className={`block px-4 pb-2 pt-5 w-full text-sm text-gray-900 bg-[#F8FAFD] rounded-sm border-2 ${
                errors.driveRole ? "border-red-500" : "border-gray-400"
              } appearance-none focus:outline-none focus:border-[#a17640] peer`}
              placeholder=" "
            ...
            <label
              htmlFor="driveRole"
              className={`px-2 text-sm ${
                errors.driveRole ? "text-red-500" : "text-gray-500"
              } duration-300 bg-[#F8FAFD] transform -translate-y-3 scale-75 top-3.5 z-10 origin-[0] left-4 peer-focus:text-xs peer-focus:text-[#a17640] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-100 peer-focus:-translate-y-6`}
            >
              Designation <span className="text-red-500">*</span>
            </label>
            {errors.driveRole && (
              <p className="text-red-500 text-xs mt-1">{errors.driveRole}</p>
            )}
          </div>
          ...
          <div className="relative mb-2">
            <input
              ...
              id="package"
              className={`block px-4 pb-2 pt-5 w-full text-sm text-gray-900 bg-[#F8FAFD] rounded-sm border-2 ${
                errors.package ? "border-red-500" : "border-gray-400"
              } appearance-none focus:outline-none focus:border-[#a17640] peer`}
              placeholder=" "
            ...
            <label
              htmlFor="package"
              className={`px-2 text-sm ${
                errors.package ? "text-red-500" : "text-gray-500"
              } duration-300 bg-[#F8FAFD] transform -translate-y-3 scale-75 top-3.5 z-10 origin-[0] left-4 peer-focus:text-xs peer-focus:text-[#a17640] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-100 peer-focus:-translate-y-6`}
            >
