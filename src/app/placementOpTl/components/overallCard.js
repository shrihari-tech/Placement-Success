...
export default function OverallCard({ title = 'Overall', studentData = [] }) {
  const { counts, totalCount, debug } = useMemo(() => {
    const initCounts = domainsList.reduce((acc, d) => ({ ...acc, [d.key]: 0 }), {});
    // defensive checks
    if (!Array.isArray(studentData)) {
      return { counts: initCounts, totalCount: 0, debug: { reason: 'not-array' } };
    }
    ...
    const hasDomainField = typeof sample.domain === 'string' && sample.domain.trim().length > 0;
    const hasBatchName = typeof sample.batchName === 'string' && sample.batchName.trim().length > 0;
    const hasBatch = typeof sample.batch === 'string' && sample.batch.trim().length > 0;
    ...
    if (hasDomainField) {
      ...
      return { counts, totalCount: Object.values(counts).reduce((a,b)=>a+b,0), debug: { detection: 'domainField', sample: studentData.slice(0,3) } };
    }
    ...
    if (hasBatchName || hasBatch) {
      ...
      return { counts, totalCount: Object.values(counts).reduce((a,b)=>a+b,0), debug: { detection: hasBatchName ? 'batchName' : 'batch', sample: studentData.slice(0,3) } };
    }

    // last resort: look for other fields that might contain course/program text
    studentData.forEach((s) => {
      const maybe = s.domain || s.course || s.program || s.center || s.batchName || s.batch || '';
      const key = mapDomainToKey(maybe);
      if (key && counts.hasOwnProperty(key)) counts[key] += 1;
    });

    return { counts, totalCount: Object.values(counts).reduce((a,b)=>a+b,0), debug: { detection: 'heuristic', sample: studentData.slice(0,3) } };
  }, [studentData]);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 border border-gray-100">
      <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center">
        ...
        {title}
        <span className="ml-2 text-lg font-bold text-gray-800">({totalCount})</span>
      </h2>

      <div className="grid grid-cols-6 gap-2 mt-4">
        {domainsList.map((domain) => (
          <div key={domain.key} className={`flex flex-col items-center justify-center p-3 rounded-lg transition-all duration-200 hover:shadow-md ${domain.bg}`}>
            <span className={`text-xs font-semibold ${domain.color} mb-1`}>{domain.label}</span>
            <span className="text-lg font-bold text-gray-800">{counts[domain.key] || 0}</span>
          </div>
        ))}
      </div>
      ...
      {totalCount === 0 && (
        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
          <div className="mb-2 font-semibold">Debug: no domain counts detected</div>
