...
const Card = ({ data, onViewDetails, serialNumber }) => {
  return (
    <div className="relative bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-lg p-4 sm:p-6 border border-gray-100 hover:shadow-xl transition-all duration-300 flex-1 min-w-[260px] w-full"> {/* Adjusted padding and min-width */}
      <div className="absolute top-0 right-0 w-16 h-16 sm:w-20 sm:h-20 bg-[#a17640]/5 rounded-bl-full"></div> {/* Adjusted size for mobile */}
      <div className="flex items-start justify-between mb-3 sm:mb-4"> {/* Adjusted margin */}
        ...
        <div className="bg-[#a17640]/10 text-[#a17640] text-[10px] sm:text-xs font-semibold px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full whitespace-nowrap"> {/* Adjusted text size and padding */}
          #{serialNumber}
        </div>
      </div>
      <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6"> {/* Adjusted spacing */}
        <div className="flex items-center">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#ece3d8] flex items-center justify-center mr-2 sm:mr-3 flex-shrink-0"> {/* Adjusted size and margin */}
            <FiUser className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#a17640]" /> {/* Adjusted icon size */}
          </div>
        ...
        <div className="flex items-center">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#ece3d8] flex items-center justify-center mr-2 sm:mr-3 flex-shrink-0"> {/* Adjusted size and margin */}
            <FiBriefcase className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#a17640]" /> {/* Adjusted icon size */}
          </div>
          <div className="min-w-0"> {/* Added to prevent text overflow issues */}
