...
const Graph = ({ title, data }) => {
  // Create unique gradient ID for each graph
  ...
  return (
    <div className="bg-white rounded-lg md:rounded-xl shadow-md p-3 md:p-4 w-full relative">
      ...
      <div
        className="absolute flex justify-center items-center text-xs md:text-sm text-gray-500 mt-2 gap-1 top-34 pb-3 rotate-270"
        style={{ top: "130px" }}
      >
...
const Graphs = ({ previousData, currentData }) => {
  return (
    <div className="bg-gray-50 p-4 rounded-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
        <Graph title="Previous Month" data={previousData} />
        <Graph title="Current Month" data={currentData} />
      </div>
    </div>
  );
};
